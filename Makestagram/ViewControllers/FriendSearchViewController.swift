...
class FriendSearchViewController: UIViewController {
    ...
    It is used to update the UI immediately upon user interaction, instead of waiting for a 
    server response
    */
    var followingUsers: [PFUser]? {
        didSet {
            /**
            the list of following users may be fetched after the tableView has displayed cells.
            In this case, we reload the data to reflect "following" status
            */
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        ParseHelper.getFollowingUsersForUser(PFUser.currentUser()!) {
            ...
            if let error = error {
                // call default error handler in case of Error
                ErrorHandling.defaultErrorHandler(error)
            }
        }
    }
